%%% THE FOURIER METHOD  FOR SOLVING THE DE-COUPLED MODEL FOR A POPULATION OF NEURONS WITHOUT WHITE NOISE
...
function [rho,V,thetas,U,sim_time,domain]=decoupled_Fourier(handles,M,flag)
...
V(1)=0.5*sum((ak(1,:)-afk(1,:)).^2+(bk(1,:)-bfk(1,:)).^2);%lyapanov function
U(1)=control_input(handles,domain,prc,dprc,V,omega,1,dt,bk(1,:),bfk(1,:),ak(1,:),afk(1,:),Ika,Ikb);
...
for i=2:length(t)
    ...
    th1=thetas(i-1,:)+kth1*dt*0.5;
    
    k1(1:M/2-1)=-omega*k.*bk(i-1,:)-U(i-1)*(Ika);
    k1(M/2:end)=omega*k.*ak(i-1,:)+U(i-1)*Ikb;
    ...
    u1=control_input(handles,domain,prc,dprc,V,omega,i,dt,bk1,bfk1,ak1,afk1,Ika,Ikb);
    
    kth2=omega+u1*(c0+ck*cos(k'*th1)+sk*sin(k'*th1));
    th2=thetas(i-1,:)+kth2*dt*0.5;
    k2(1:M/2-1)=-omega*k.*bk1-u1*Ika;
    k2(M/2:end)=omega*k.*ak1+u1*Ikb;
    ...
    u2=control_input(handles,domain,prc,dprc,V,omega,i,dt,bk2,bfk2,ak2,afk2,Ika,Ikb);
    
    kth3=omega+u2*(c0+ck*cos(k'*th2)+sk*sin(k'*th2));
    th3=thetas(i-1,:)+kth3*dt;
    k3(1:M/2-1)=-omega*k.*bk2-u2*Ika;
    k3(M/2:end)=omega*k.*ak2+u2*Ikb;
    ...
    u3=control_input(handles,domain,prc,dprc,V,omega,i,dt,bk3,bfk3,ak3,afk3,Ika,Ikb);
    
    kth4=omega+u3*(c0+ck*cos(k'*th3)+sk*sin(k'*th3));
    thetas(i,:)=thetas(i-1,:)+dt*(kth1+2*kth2+2*kth3+kth4)/6;
    k4(1:M/2-1)=-omega*k.*bk3-u3*Ika;
    k4(M/2:end)=omega*k.*ak3+u3*Ikb;
    ...
    U(i)=control_input(handles,domain,prc,dprc,V,omega,i,dt,bk(i,:),bfk(i,:),ak(i,:),afk(i,:),Ika,Ikb);

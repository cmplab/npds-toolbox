...
function [phi,L2,thetas,u,sim_time,x]=noisy_decoupled_spectral(handles,M,flag)
...
%D^1 and D^2 matrices
w=eye(N);
Q=((2/x(end))-1)*eye(N);
Q_inv=1/((2/x(end))-1)*eye(N);
wd=diff_matrix.DJ;
wdd=(Q*wd)*Q_inv*wd;
...
for i=1:S
    ...
    thetas(i+1,:)=RK_stoc(thetas(i,:),u(i+1),T,dt,D,model,'Spectral');

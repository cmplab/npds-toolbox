%%% 3D STREAM  FOR SYSTEM OF FIRST ORDER ODES
% NPDSToolbox version 1.0.0
%       stream3d(func,y1val,y2val,y3val,t) plots the stream for the system of
...
function stream3d(H,func,y1val,y2val,y3val,t,varargin)
n1=length(y1val);
n2=length(y2val);
n3=length(y3val);
yp1=zeros(n2,n1);
    ...
    for i=1:n1
        for j=1:n2
            for k=1:n3
                ...
                yp1(k,j,i) = ypv(y0_idx(1));
                yp2(k,j,i) = ypv(y0_idx(2));
                yp3(k,j,i) = ypv(y0_idx(3));
            end
        end
    end
...
if sqrt((max(y1val)-min(y1val)).^2+(max(y2val)-min(y2val)).^2+(max(y3val)-min(y3val)).^2)>10
    arrow='noarrows';
end
[Y1val,Y2val,Y3val]=meshgrid(y1val,y2val,y3val);
q=streamslice(H,Y1val, Y2val, Y3val, yp1, yp2, yp3, max(max(max(Y1val))), max(max(max(Y2val))), min(min(min(Y3val))),arrow,'nearest');
axis tight;
